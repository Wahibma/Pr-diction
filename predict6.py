...
with col1:
    ...
    fig, ax = plt.subplots()
    ax.plot(y_test.values, label='True Values', marker='o')
    ax.plot(models['Régression Linéaire'][1], label='Régression Linéaire', marker='x')
    ax.plot(models['Forêt Aléatoire'][1], label='Forêt Aléatoire', marker='s')
    ax.plot(models['Gradient Boosting'][1], label='Gradient Boosting', marker='d')
    ax.legend()
